...
<body class="flex items-center justify-center min-h-screen bg-gray-100 p-4">
    <div class="bg-white rounded-lg shadow-lg w-full max-w-4xl flex flex-col md:flex-row border border-black overflow-hidden">
        <div class="w-full md:w-1/2 p-6">
            ...
            <form id="loginForm" class="space-y-3" method="POST" action="{{ route('check.signup') }}">
                @csrf
                <div>
                    @if (session('error'))
            ...
            <p class="text-center text-blue-500 underline">انتقل <a href="{{ route('login') }}" >لتسجيل الدخول</a></p>
        </div>
