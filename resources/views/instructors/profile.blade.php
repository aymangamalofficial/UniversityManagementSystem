        ...
        <nav class="bg-gray-800 text-white py-4 px-6 flex justify-between items-center shadow-lg fixed top-0 left-0 w-full">
            ...
            <div class="hidden md:flex items-center space-x-6" id="nav-links">
                <ul class="flex space-x-6">
                    <li><a href="{{ route('instructors.dash')}}" class="hover:text-blue-300">إدارة  المواد</a></li>
                </ul>
            </div>
<!-- button set,not,user -->
            <div class="relative mt-2 grid grid-cols-3 sm:grid-cols-3 lg:grid-cols-3 gap-3">
                ...
                <div class="relative">
                    ...
                    <div id="dropdown-menu" class="hidden absolute left-0 mt-3 w-48 bg-white text-black shadow-lg rounded-lg py-2 z-50">
                        <a href="{{ route('instructors.profile')}}" class="block px-4 py-2 hover:bg-gray-200">الملف الشخصي</a>
                        <a href="{{ route('home')}}" class="block px-4 py-2 hover:bg-red-500 text-red-700 hover:text-white">تسجيل الخروج</a>
                    </div>
                </div>
            </div>
        </nav>
    ...
    <aside id="sidebar" class="fixed md:hidden top-10 right-0 h-screen bg-blue-100/75 transform translate-x-full transition-transform duration-300 z-50 md:translate-x-0">
        <div class="sidebar mt-16">
            <ul>
                <li class="mb-4"><a href="{{ route('instructors.dash')}}" class="block px-3 py-1 text-black hover:bg-pink-950 hover:text-blue-200"> المواد </a></li>
                <li class="mb-4"><a href="{{ route('instructors.result')}}" class="block px-3 py-1 text-black hover:bg-pink-950 hover:text-blue-200"> الدرجات </a></li>
                <li class="mb-4"><a href="{{ route('instructors.assignment')}}" class="block px-2 py-1 text-black hover:bg-pink-950 hover:text-blue-200"> الأسيمنتات </a></li>
                <li class="mb-4"><a href="{{ route('instructors.absence')}}" class="block px-3 py-1 text-black hover:bg-pink-950 hover:text-blue-200"> تسجيل الغياب </a></li>
            </ul>
        </div>
    </aside>
    ...
    <div class="grid md:grid-cols-2 gap-8 items-center flex-row-reverse">
        <div class="genral">
            <h1 class="text-2xl font-bold text-pink-950 leading-tight text-center md:text-right">
                مرحباً مهندس <span class="text-blue-900">{{ $instructor->name }}</span> في جامعة أسيوط الجديدة التكنولوجية
            </h1>
            <div class="bg-gray-50 shadow-md m-10 rounded-2xl p-5 border border-gray-300">
                <div class="relative flex items-center space-x-3 mb-4 text-base sm:text-lg md:text-xl">
                    <div class="bg-blue-500 text-white p-3 rounded-full">
                        <img src="{{ asset('public/resources/instructors/profile/user.png') }}" alt="" class="w-5 h-5">
                    </div>
                    <h4 class="text-red-900 text-2xl">بشمهندس/ة :</h4>
                    <h4 class="text-gray-700 text-2xl">{{ $instructor->name }}</h4>
                </div>
                <ul class="space-y-3">
                    <li class="flex items-center space-x-2"><i data-lucide="id-card" class="w-5 h-5 text-blue-500"></i><span class="text-red-900 font-medium">الرقم القومي :</span><span class="text-gray-700">{{ $instructor->national_id }}</span></li>
                    <li class="flex items-center space-x-2"><i data-lucide="key" class="w-5 h-5 text-blue-500"></i><span class="text-red-900 font-medium">الكود الجامعي :</span><span class="text-gray-700">{{ $instructor->code }}</span></li>
                    {{-- <li class="flex items-center space-x-2"><i data-lucide="phone" class="w-5 h-5 text-blue-500"></i><span class="text-red-900 font-medium">رقم الهاتف :</span><span class="text-gray-700">{{ $instructor->phone }}</span></li> --}}
                    {{-- <li class="flex items-center space-x-2"><i data-lucide="file-user" class="w-5 h-5 text-blue-500"></i><span class="text-red-900 font-medium">التخصص :</span><span class="text-gray-700">{{ $instructor->specialization }}</span></li> --}}
                    <li class="flex items-center space-x-2"><i data-lucide="at-sign" class="w-5 h-5 text-blue-500"></i><span class="text-red-900 font-medium">الإيميل :</span><span class="text-gray-700" dir="ltr">{{ $instructor->email }}</span></li>
                </ul>
            </div>
        </div>
        <div class="flex justify-center">
            <img src="{{ asset('public/resources/instructors/profile/administrative-assistant.gif') }}" alt="Student Image" class="w-80 h-80">
        </div>
    </div>
<!-- settingsModal -->
    ...
    <div class="bg-white top-10 grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-2 gap-3 rounded-xl shadow-lg w-full max-w-md mx-auto p-6 relative overflow-y-auto max-h-[90vh]">
        ...
        <div class="flex flex-col items-center mb-6">
            <label for="profileImageInput" class="cursor-pointer">
                <img id="previewImage" src="{{ asset('public/resources/instructors/profile/user.png') }}" alt="الصورة الشخصية" class="w-20 h-20 rounded-full border shadow-md object-cover">
            </label>
        ...
        <form onsubmit="return false">
            <div class="mb-4">
                ...
                <input type="text" value="محمود محمد" disabled class="mt-1 border block w-full rounded-md border-gray-300 shadow-sm" />
            </div>
            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700">الرقم الجامعي</label>
                <input type="text" value="{{ $instructor->code }}" disabled class="mt-1 border block w-full rounded-md border-gray-300 shadow-sm" />
            </div>
            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700">رقم الهاتف</label>
                <input id="phoneInput" type="text" value="{{ $instructor->phone }}" class="mt-1 border block w-full rounded-md border-gray-300 shadow-sm" />
...
<script src="{{ asset('public/resources/instructors/profile/assit.js') }}"></script>
